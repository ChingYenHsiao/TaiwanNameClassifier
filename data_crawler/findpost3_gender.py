...
def get_post_comment(result_path,Post_dict,graph):
    ...
    PageID = '851867474870404'
    PostID = PageID+'_'+'1528985553825256'
    #PostID = '330133697344117_433555783668574'
    since="2017-06-01"
    until="2017-07-04"
    #result_comment =  './name/name_'+PostID+"_"+since+"_"+until+'.csv'#result_path + page_id + "_comment_reaction.csv"
    result_comment =  './name_gender/name_'+PostID+'.csv'#result_path + page_id + "_comment_reaction.csv"
            ...
            for comment in next_ob:
                ...
                if 'comments' in comment:
                    for index,comment_coment in enumerate(comment['comments']['data']):
                        if(len(comment_coment['message'])!=9):
                            continue
